...
|-----|------|-------------|
| CWE-787 | Out-of-bounds Write | `unsafe.|Buffer\.alloc\(.*\)` |
| CWE-125 | Out-of-bounds Read | N/A — C/C++ specific; manual review recommended for JS/TS, Python, C#, Go |
| CWE-416 | Use After Free | N/A — C/C++ specific; not applicable to JS/TS, Python, C#, Go |
...
| CWE | Name | Grep Pattern |
|-----|------|-------------|
| CWE-476 | NULL Pointer Deref | Manual review — property access patterns (`.Length`, `.Count`, `.size()`) without null checks are not reliably grep-detectable |
